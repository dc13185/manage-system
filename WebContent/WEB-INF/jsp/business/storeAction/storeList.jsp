					...
					<div class="modal-body">
						 <form role="form">
                              ...
                              <div class="form-group">
                                  <label>店铺名</label>
                                  <input type="text" id="storeName" name="storeName" placeholder="请输入店铺名" class="form-control">
                              </div>
                              <div class="form-group">
                                  <label>图片</label><br>
                              ...
                              <div class="form-group">
                               ...
                               <div class="form-group">
                                   ...
                                   <select class="form-control m-b" name="account" id="storeType">
                                   		 ...
                                   		 <c:if test="${! empty storeAdminTypes}">
                                   		 	<c:forEach items="${storeAdminTypes}"  var="storeAdminType">
                                   		 			<option value="${storeAdminType.storeType_id}">${storeAdminType.storeType_name}</option>
                                   		 	</c:forEach>
                                   		 </c:if>
                                   </select>
                              </div>
                                ...
                                <div class="form-group">
                                   <label>绑定用户</label>
                                   <select class="form-control m-b" name="account" id="storeUserID">
                                    	 <option value="">请选择店铺绑定用户</option>
                                   		 <c:if test="${! empty tenanUsers}">
                                   		 	<c:forEach items="${tenanUsers}"  var="systemUser">
                                   		 			<option value="${systemUser.user_id}">${systemUser.username}</option>
                                   		 	</c:forEach>
                                   		 </c:if>
                                   </select>
                              </div>
		...
		var TableInit = function () {
			...
			oTableInit.Init = function () {
				$('#exampleTableEvents').bootstrapTable({
					url:'${_ctx}/storeAction/findStoreListByPage',         //请求后台的URL（*）
					method: 'post',                      //请求方式（*）
					...
					columns: [
					          ...
					          {title:'店铺名',field: 'store_name',sortable:true ,align:"center" },
					          {title:'店铺图片',field: 'store_image',align:"center",sortable:true ,formatter: function (value, row, index) {
					        	  return [
											'<img  src="'+row.store_image+'?='+Math.random()+'" style="width:150px;" >'
											].join("") 
				              }
					          },
					          {title:'店铺拥有者',align:"center",align:"center",field: 'store_user_name'},
					          {title:'店铺类型',align:"center",align:"center",field: 'storeType_name'},
					          {title:'总收入额',align:"center",field: 'all_money', formatter: function (value, row, index) {
					        	  if(value){
					          ...
					          {title:'店铺状态',align:"center",align:"center",field: 'status',formatter: function (value, row, index) {
					        	 	 var status;
					        	 	 if(value==2){status="未开张"}else if(value==1){status="开张"}else{status="冻结"}
					        	 	 return status;
					          }},
					          {title:'操作',field:"Button",align:"center",formatter:function (value, row, index) {
					        	  return [
						        	        ...
						        	        '<button class="btn btn-info btn-xs kzButton " style="cursor: pointer;margin-left: 10px;" type="button">开张/关门</button>'+
						        	        '<button class="btn   btn-xs btn-danger djButton" style="cursor: pointer;margin-left: 10px;" type="button">冻结/激活</button>'
											].join("")
